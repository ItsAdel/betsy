...
async function approveUSDC(amount: bigint) {
  try {
    ...
    const approveArgs = [binaryBettingAddress, approveAmount.toString()];

    const contractInvocation = await groupWallet.invokeContract({
...
async function createGame(
  ...
  try {
    ...
    ]; // Arguments for createGame

    // Call the contract method using invokeContract
    const contractInvocation = await groupWallet.invokeContract({
      contractAddress: binaryBettingAddress, // Address of your binary betting contract
      method: "createGame", // The method to call
      args: createGameArgs, // Arguments for the createGame function
    });

    // Wait for the transaction to complete
    const receipt = await contractInvocation.wait();
    console.log("Game created, transaction receipt:", receipt);
  } catch (error) {
...
async function resolveGame(gameId: number, yesWon: boolean) {
  try {
    const resolveGameArgs = [gameId, yesWon]; // Arguments for resolveAndDistribute

    // Call the contract method using invokeContract
    const contractInvocation = await groupWallet.invokeContract({
      contractAddress: binaryBettingAddress, // Address of your binary betting contract
      method: "resolveAndDistribute", // The method to call
      args: resolveGameArgs, // Arguments for resolveAndDistribute
    });

    // Wait for the transaction to complete
    const receipt = await contractInvocation.wait();
    console.log(
      `Bet ${gameId} resolved with outcome: ${yesWon ? "Yes" : "No"}`
    );
  } catch (error) {
    console.error("Error during resolveGame:", error);
  }
}
...
const coinbase = new Coinbase({
  apiKeyName: process.env.CDP_API_NAME as string,
  privateKey: process.env.CDP_API_PRIVATE_KEY!.replaceAll(
    "\\n",
    "\n"
  ) as string,
});

let groupWallet: Wallet;

// Initialize the group wallet using Coinbase SDK
async function initializeCoinbaseWallet() {
  groupWallet = await Wallet.create();
  console.log(
    `Coinbase wallet created with address: ${groupWallet.listAddresses()}`
  );
}

async function faucetCoinbaseWallet() {
  await groupWallet.faucet();
  console.log("Faucet operation for Coinbase wallet complete");
}
...
export async function handler(context: HandlerContext) {
  if (!groupWallet) {
    await initializeCoinbaseWallet();
    await faucetCoinbaseWallet();
  }
